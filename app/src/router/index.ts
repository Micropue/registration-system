...
const ADMIN_PERMISSION_KEYS = ['账户管理', '账户组管理', '订单处理', '工单处理', 'APP配置', '充值审批', '下属管理', '公告管理', '日报管理']
...
/**
 * 路由守卫：要求必须拥有指定模块的操作权限
 */
function checkAdminPerm(module: string, action: string): NavigationGuard {
  return async (_to, _from, next) => {
    const userInfo = await checkLoginStatus()
    if (!userInfo) return next('/login')
    const perm = userInfo.permissions?.[module]
    if (typeof perm === 'object' && perm !== null && perm[action]) return next()
    return next('/')
  }
}
